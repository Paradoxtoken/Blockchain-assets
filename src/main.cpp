...
#if defined(NDEBUG)
# error "Paradox cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x261c9d948358e84544e02c29251d5ecfe620efd01dc88f4066d1c64a8d57adad");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Paradox: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Paradox Signed Message:\n";
...
{
    // Paradox: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Paradox: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Paradox: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1498619410, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "28 06 2017 SKY NEWS UK Boy with awful injuries dies at rail depot";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047bee3c32419a1c76ad3c0fff88a1979e789f024611b2a0b2a109abe2f696becf5b47301ab5f992bce15cc7ba68fe4ee9191c4346304c9d6b2817b4be5200c016") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1498619410;
        ...
        assert(block.hashMerkleRoot == uint256("0x2f11e23f0a56e6cadc9f7dddf81cff71729e19b9df39ef8312b6bf6c4941f7b5"));
...
unsigned char pchMessageStart[4] = { 0x50, 0xa6, 0x16, 0x91 }; // Paradox: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ParadoxMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ParadoxMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ParadoxMiner(CWallet *pwallet)
{
    printf("ParadoxMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("paradox-miner");
    ...
    try { loop {
        ...
        printf("Running ParadoxMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ParadoxMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ParadoxMiner, pwallet));
}
